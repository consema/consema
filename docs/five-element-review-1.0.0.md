...
- 核验口径：
  ...
  - **工作树** = 主仓库当前状态。版本推进（15 个 `Cargo.toml` + `go/cmd/consema/version.go` 的
    `0.13.0 → 1.0.0-rc.1`）与 RFC 0020 已于 2209582 提交（docs/rfcs/0020-compatibility-and-support-policy-v1.md，
...
| Go CLI 合入后的 cross-language exchange 复跑（P2-7） | 已闭环（2026-08-10，rc-1.0.0-candidate.md:193：G5.6 合入后四 harness 复跑 83/83 + 108/108 + 68/68） | §22.2（rc-1.0.0-candidate.md:38 边界）→ §28.5 条件 4 的 CLI 侧核对 | 不阻塞（SDK 面 83/83 已实测；CLI 侧已闭环） |
...
核验对象：提交态 74e336e（历史提交，当时 Rust workspace 0.13.0 / Go CLI product_version 0.19.0）。本表为修复前实测记录——F-A/F-B 已由 2209582 处置并审计验证。
...
- **当前状态（已修复）**：按原选项 (a) 执行——RFC 0015 §3.3 已修订（docs/rfcs/0015-cli-machine-protocol-and-batch-apply-v1.md:184-193，
  ...
  （crates/consema-protocol/src/cli.rs:870-911）与 Go `isSemanticVersion`（go/protocol/cli.go:1012-1044），
  错误文案现为 "expected MAJOR.MINOR.PATCH[-prerelease] without leading zeros or build metadata"
  （cli.rs:197,321 / cli.go:799,912）。`1.0.0-rc.1` 已提交（Cargo.toml:31 / go/cmd/consema/version.go:15）。
...
- **当前状态（已修复）**：按原选项 (a) 执行——从干净 LF checkout 重算 digest = **35bebc8d384d…**，已回填
  fc-manifest-0.13.0.json:38（`conformance_suite.aggregate_sha256`）与 go/conformance/conformance_test.go:106
...
- **补注（本机差异，文档化）**：本机 CRLF 工作树（`core.autocrlf=true`）上仍算出 e3d6578858…，属文档化
  本机差异（rc-1.0.0-candidate.md:193 同述）；规范 checkout（LF）为权威口径，CI 不受影响。
- **修复前状态（历史记录）**：
  - 现象：干净 checkout（LF，`.gitattributes` 第 1 行 `* text=auto eol=lf`）下
    ...
    `scripts/go-verify-shared-conformance.ps1` 步骤 [1/6] FAIL：computed `35bebc8d384d…` ≠
...
7. **P2-7**：Go CLI 合入后 exchange 复跑闭环——已闭环（rc-1.0.0-candidate.md:193，2026-08-10）；
...
- F-B 已修复（2209582）：干净 LF checkout 重算 35bebc8d… 并回填，"记录值在记录机（CRLF）复现"与
  "干净 checkout 不可复现"两侧差异现为文档化本机行为（§4 第 12/13 行，rc-1.0.0-candidate.md:193），
