...
- **向量变更是五仓同步事件**：任何一仓修改 `conformance/vectors/` 必须同步全部五个语言仓并更新聚合 digest 与 18/519 计数（README 声明；未同步会让 go/ts/kt 三仓 conformance gate 与 digest 断言失败——consema-rs 为 vendored 快照、consema-py CI 钉定 commit，两仓不自动跟随）。
