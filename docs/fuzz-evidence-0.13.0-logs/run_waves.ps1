...
# (runs.csv in this directory). This is the executable form of the accumulation
# protocol documented in docs/fuzz-evidence-0.13.0.md; see that file for the
# protocol, the honest-accounting rules and the classification table.
#
# Usage (from the repository root):
#   powershell -ExecutionPolicy Bypass -File docs\fuzz-evidence-0.13.0-logs\run_waves.ps1 -Waves 16 -Copies 2
#
# Behavior per wave:
...
# (crates/consema-conformance/src/fuzz.rs; cargo-fuzz targets are the
# clang-host completion path, see docs/fuzz-evidence-0.13.0.md).
...
# crates/consema-conformance/tests/*_fuzz.rs): parse_fuzz 100_000 iters x 4
# bases = 400_000 mutations per target; operation_fuzz 25_000 x 3 bases =
# 75_000; protocol_fuzz 100_000 x 4 bases x 2 seeds = 800_000.
$parseTargets = @(
