...
- 对应计划：`docs/0.13.0-gate-plan.md` M8（72 CPU-hours release-candidate fuzz，串行，依赖 M2）与 §15.3 质量门禁（路线图 §15.3「质量门禁」节——行号可能漂移，以节标题为锚）
- 缺陷分级：路线图 §18.4「缺陷等级」（P0 数据破坏/静默损失/RCE/错误写文件/跨快照误编辑；P1 panic/crash/hang/错误完成状态/明显语义不一致/limit bypass；P2 有安全替代路径的功能缺陷/非核心性能回退/诊断位置错误——行号可能漂移，以节标题与分级定义句为锚）
...
> **六仓拆分影响说明（2026-08-12 追加，2026-08-15 修订）**：六仓拆分后，fuzz 驱动运行于 **consema-rs** 仓根（`run_waves.ps1` 运行副本——已入库跟踪：consema-rs commit d97a038，2026-08-13（记录时点为本地未跟踪副本）；新增 `-LedgerDir` 参数，默认指向母仓 `docs/fuzz-evidence-0.13.0-logs` 绝对路径；账本目录副本为拆分时点快照 + 2026-08-13 母仓 commit 8f1ffa2 追加（CONSEMA_GIT_EXE override，+13 行），非冻结原版——「冻结协议证据」声称撤销）；账本与 waves.log 仍写本仓 `docs/fuzz-evidence-0.13.0-logs/`——C-2 证据权威不变；runs.csv 为追加式账本（只 append、从不改写），拆分不触碰既有行，累计连续性不受影响；C-2 门禁单位口径不变（72 CPU-hours/格式家族 = parse+ops 合计、protocol 单独，§15.3）。会话 455 起由 consema-rs 驱动续接，详情见 §3.1 六仓拆分停机 incident 记录。

> **驱动暂停状态（2026-08-15 追加，诚实记录）**：驱动自 2026-08-13 11:19:44 session 916 无记录终止后未重启（waves.log 止于 `wave 1 start: 34 processes`，runs.csv 冻结于 122,478 文件行 / 780.529 CPU-hours）；session 916 修复 commit（consema-rs 5d010aa，2026-08-14 10:52）落地后亦未重启。toml/protocol-decode/plist/xml 四单位未过 72h 门槛，其累计停摆如实记录为遗留；重启与累计恢复见 §7 完成路径。
...
- `exit_code`：0 = 全部断言通过；101 = 测试失败（= 找到 violation，P0/P1 事件）；-998 = 观测竞态未能取码（按失败处理并人工复核日志）；-997 = 驱动检出失败（G037：进程 exit 0 但 outlog 无 `test result: ok. 1 passed` 行——libtest 过滤器零匹配仍 exit 0，按失败处理并人工复核 outlog，见 §3.1 session 916 注记）；-1000 = 波次安全超时强杀（hang 候选，P1 事件）；**-1 = 进程外部终止（operator 停止/停机中断时驱动对进程 ExitCode 的占位值——退出码未被观测，原样写入；账本中 24 行 = session 9 wave 3 的 10 行 + session 454 wave 4 的 14 行，均按失败处理并人工复核，CPU 真实计入，非 fuzz finding）**。
...
|---|---|---|---|
| 1-6 | 14:49-14:57 | 驱动脚本开发/排障（Start-Process 在该 .NET Framework 宿主上损坏 Process.CPU/ExitCode/HasExited 读取，已改用直接 .NET ProcessStartInfo 模式；问题与修复全程记录在 waves.log） | 无有效账本行（waves.log 该时段记录 34 条 FAIL 行——exit 字段为空、cpu=0s、wall≈61-66s 整波超时，且两次 `session done` 汇总 failures=17/17（"17/34 ledger rows; total CPU-hours=0"）——观测失效期间运行未入账、不产生时长；本行原"两波已运行测试全部通过"叙述与 waves.log 的 FAIL 行记录矛盾，按账本实况修正） |
...
> **恢复与迁移**：11:41:04 session 455 起续接（consema-rs HEAD=6bcb068，重建后 wave 1 正常入账），停机 42 分钟（10:58:58 → 11:41:04）；驱动迁移至 consema-rs 根——`run_waves.ps1` 运行副本（未跟踪文件；记录时点为未入库，2026-08-13 已随 consema-rs d97a038 入库跟踪），新增 `-LedgerDir` 参数默认指向母仓 `docs/fuzz-evidence-0.13.0-logs` 绝对路径，tree-hash pathspec 由 `crates Cargo.toml Cargo.lock conformance` 更新为根布局 `Cargo.toml Cargo.lock conformance 'consema*'`；原版脚本保留于账本目录作冻结协议证据（该副本后经 8f1ffa2 追加 CONSEMA_GIT_EXE override，非冻结原版——见 §3.1 驱动暂停状态）；账本与 waves.log 仍写母仓路径，C-2 证据权威不变。
...
> **2026-08-12 17:44-17:50 构建失败循环 incident（追加，诚实记录）**：
> 17:41:01 session 559 正常启动后，17:44:09 起的 18 个连续会话（560-577）全部在 wave 1 构建阶段失败（waves.log 逐条 `wave 1: cargo build failed (exit 101) or binaries missing; aborting (exit 2)`，每会话 1 条，间隔约 22-26s）——零行入账、会话计数膨胀 18；根因系该时段驱动重建时的构建环境问题（cargo build 失败循环，非 fuzz finding，不影响任何 target 的 clean 计时、无 corpus regression）。17:52-17:53 恢复（session 578 build ok，`session done: 77629 ledger rows; total CPU-hours=542.643 failures=0`）。该事件已按 §4.4 协议补记本节时间线（原时间线最后条目为 session-454 停机 incident，未覆盖本事件）。

> **2026-08-13 session 908-913 构建失败循环 incident（追加，诚实记录）**：
> 2026-08-13 11:04:46-11:07:15 session 908-913 六个连续会话全部在 wave 1 构建阶段失败（waves.log 第 16277-16326 行，逐条 `wave 1: cargo build failed (exit 101) or binaries missing; aborting (exit 2)`，间隔约 24-35s）——零行入账、会话计数膨胀 6；session 914（11:07:37）构建恢复（build ok）后 wave 1 正常入账。根因系该时段驱动重建时的构建环境问题（非 fuzz finding，不影响任何 target 的 clean 计时、无 corpus regression）；与 2026-08-12 session 560-577 构建失败循环同类（waves.log 第 10244 行起——session 560 start 实测位置，session 577 start 于第 10329 行；2026-08-15 波 5 归正此前错引的「第 13433 行」（该行为 session 750 start，无关）；行号可能漂移，以 session= 起始行为锚）。该事件已按 §4.4 协议补记本节时间线。

> **2026-08-10 session 62/63/148/171 中止簇（追加，诚实记录）**：
> 2026-08-10 14:55:30/14:56:41 session 62/63（waves.log 第 1335/1341 行）wave 1 启动后无 wave done 行即被中断（零行入账）；19:41:30 session 148（waves.log 第 2842 行）wave 1 构建于 19:44:17 中止（exit 101，零行入账）；20:53:46 session 171（waves.log 第 3238 行）wave 1 启动后被中断（零行入账）。全部为外部中断/构建中止、非 fuzz finding，不影响任何 target 的 clean 计时、无 corpus regression（§4 step 2）；与已记录的 session 90/166/170/179/180/187/188 中断体例一致。

> **2026-08-10/2026-08-12 六个未记录会话中断簇（追加，诚实记录，2026-08-15 波 5）**：
> session 13/51/77/144/167/600（waves.log 第 453/1137/1581/2769/3169/10735 行起）各含
> 真实证据行（runs.csv 合计 510 数据行）但在 waves.log 中块均以 wave start/done 后静默
> 中断——无 session done、无 FINAL、无 ABORT（session 600 以 `wave 2: cargo build failed
> (exit 101) or binaries missing; aborting (exit 2)` 收尾，同样无 session done）；与已记录
> 的 session 62/63/148/171「wave 1 启动后无 wave done 行即被中断」为同一中断类别，但此前
> 事件时间线未枚举。全部为外部中断/构建中止、非 fuzz finding，不影响任何 target 的 clean
> 计时、无 corpus regression（§4 step 2）。行号可能漂移，以 session= 起始行为锚。

> **2026-08-13 session 916 无记录终止 incident（G20-255，追加，诚实记录）**：
> 2026-08-13 11:19:44 session 916（waves.log 第 16343 行起）wave 1 启动 34 进程后驱动无记录终止——waves.log 止于 `wave 1 start`，无 wave done/session done 行、无 ABORT 行，runs.csv 零行入账（`session done: 122477 ledger rows` 已由前一会话 915 于 11:19:09 落账）。**已诊断并修复**（rs 波 2 审计）：根因为 PowerShell 脚本块 DataReceived 回调在 .NET 线程池上的未处理 PSInvalidOperation（`ScriptBlock.GetContextFromTLS` 竞争；WER 签名、exit code 2 无记录）——与 session 916 的终止同类；修复已落地于 consema-rs 的 `run_waves.ps1`（consema-rs commit 5d010aa，2026-08-14 10:52 提交；本注记写于修复提交前，修复后驱动未重启——见 §3.1 驱动暂停状态）：异步排空改为编译型 C# 委托（无 runspace 上下文，该崩溃类不再存在），并新增 ABORT 路径记录（会话未达 FINAL——强杀/未处理错误/提前退出——必须留下显式 ABORT 行，不再静默断档）。非 fuzz finding：不影响任何 target 的 release-candidate clean 计时、无 corpus regression（§4 step 2）。
...
### 3.2.1 最新复算快照（追加；runs.csv 为唯一权威账本）
**2026-08-13 复算快照（runs.csv 为唯一权威账本，按 §4.4 step 4 协议更新）**：账本 122,477 数据行 / 122,478 文件行（含表头 1 行：122,477 数据行 + 表头 = 122,478 文件行；2026-08-15 波 5 归正括注——此前「122,477 均指含表头的 122,478 口径」自相矛盾）/ **780.529 CPU-hours**（122,437 行 exit 0；非零退出 40 行 = session-9 的 10 行外部终止 + session-448 wave 3 的 16 行超时 + session-454 的 14 行六仓拆分停机中断，均非 fuzz finding，见 §3.1 事件时间线）。每格式门槛单位累计（CPU-hours，本机复算）：properties **145.4**（201.9%，已过 72h）/ yaml **128.8**（178.9%，已过 72h）/ ini **124.8**（173.3%，已过 72h）/ hcl **95.5**（132.7%，已过 72h）/ json **95.3**（132.3%，已过 72h）/ toml **57.7**（80.2%）/ protocol-decode **50.7**（70.4%）/ plist **47.4**（65.8%）/ xml **35.0**（48.6%）。properties/yaml/ini/hcl/json 五单位已过 72h 门槛；toml 为其余中最接近门槛的单位；其余单位未过门槛，按 §7 完成路径累计（clang 主机 cargo-fuzz 为主，本机连续 wrapper 续跑为备选）；runs.csv 为唯一权威账本；**驱动已暂停（2026-08-13 11:19 session 916 后未重启），数字冻结于本快照——四单位未过 72h 门槛如实记录为遗留（见 §3.1 驱动暂停状态）**。
**2026-08-12 13:43 快照（session 492 done；runs.csv 为唯一权威账本）**：账本 68,415 行 / **488.634 CPU-hours**（68,375 行 exit 0；非零退出 40 行 = session-9 的 10 行外部终止 + session-448 wave 3 的 16 行超时 + session-454 的 14 行六仓拆分停机中断，均非 fuzz finding，见 §3.1 事件时间线）。每格式门槛单位累计（CPU-hours，本机复算）：properties **92.0**（127.8%，已过 72h）/ yaml **81.5**（113.2%，已过 72h）/ ini **78.4**（108.9%，已过 72h）/ hcl **59.8**（83.1%）/ json **59.4**（82.5%）/ toml **36.0**（49.9%）/ protocol-decode **31.4**（43.6%）/ plist **28.9**（40.2%）/ xml **21.2**（29.5%）。properties/yaml/ini 三单位已过 72h 门槛；hcl/json 为其余中最接近门槛的单位；其余单位按 §7 完成路径累计（clang 主机 cargo-fuzz 为主，本机连续 wrapper 续跑为备选）；runs.csv 为唯一权威账本，当时驱动中（session 493+ 持续追加；该累计已于 2026-08-13 11:19 冻结，见 §3.1 驱动暂停状态）。
...
1. 六仓拆分后从 consema-rs 根运行：`powershell -NoProfile -ExecutionPolicy Bypass -File run_waves.ps1 -Waves <N> -Copies <C>`（运行副本——已入库跟踪：consema-rs commit d97a038；`-LedgerDir` 默认指向母仓 `docs\fuzz-evidence-0.13.0-logs` 绝对路径；账本目录副本含拆分后变更（8f1ffa2，CONSEMA_GIT_EXE override），协议权威 = consema-rs 根入库副本；脚本是 §3.3 协议的机器可执行形式；每波自动 tree-hash → 重建 → 并发运行 → 逐进程真实 CPU/墙钟/退出码 → 追加 runs.csv 与 waves.log）。
...
新发现协议：P0/P1 = 立即报告 + 最小输入入 corpus + 清零该 target 时钟；P2 = 记录并给发布判断。`1.0.0` 不允许未解决 P0/P1（路线图 §18.4「缺陷等级」节——行号可能漂移，以节标题为锚）。
...
- **corpus 全绿**：`cargo test -p consema-conformance --test mutation_corpus --locked -- --ignored` 全量 174,921 case replay 通过（2026-08-07 15:49）。（证据链说明（2026-08-11 审计追加）：workspace-gate-2026-08-07.log 存在（15:48:41 写入）但未含该全量 replay 的通过记录——该日志中 `mutation_corpus_replay_full` 标记为 ignored（manual evidence run: full 174,921-case replay）、仅 `mutation_corpus_replay_bounded` 通过（2.57s）；63.10s 时长与全量 replay 的原始输出未保留在仓内，**不可复算**——本声明以当日 15:49 的 `-- --ignored` 运行记录为准，时长数字不再引用（与五要素复核 174,921 case 同源））
...
- **2026-08-13 复算快照（runs.csv 为唯一权威账本，发布记录数字以本快照为准；按 §4.4 step 4 协议更新）**：账本 122,477 数据行（122,478 文件行含表头）/ **780.529 CPU-hours**（122,437 行 exit 0、40 行非零退出——10 行 session-9 外部终止 + 16 行 session-448 wave 3 超时 + 14 行 session-454 六仓拆分停机中断，均非 fuzz finding，见 §3.1 事件时间线）。每格式门槛单位累计（CPU-hours，本机复算）：properties **145.4**（201.9%，已过 72h）/ yaml **128.8**（178.9%，已过 72h）/ ini **124.8**（173.3%，已过 72h）/ hcl **95.5**（132.7%，已过 72h）/ json **95.3**（132.3%，已过 72h）/ toml **57.7**（80.2%）/ protocol-decode **50.7**（70.4%）/ plist **47.4**（65.8%）/ xml **35.0**（48.6%）。properties/yaml/ini/hcl/json 五单位已过 72h 门槛；toml 为其余中最接近门槛的单位；其余单位仍开放，剩余部分按 §7 完成路径累计（clang 主机 cargo-fuzz 为主，本机连续 wrapper 续跑为备选）；runs.csv 为唯一权威账本；驱动已暂停（2026-08-13 11:19 后未重启），数字冻结。

## 9. 开放事项

- consema-rs 的 `consema-conformance/examples/gen_mutation_corpus.rs`（拆分前 `crates/consema-conformance/examples/gen_mutation_corpus.rs`）regressions 保留事项已解决：修复已随 commit 92a244a 落地——`generate()` 接收 regressions 文本并原样写入输出（不再硬编码 `[]`），`extract_regressions` 从已提交 corpus 逐字提取既有数组，新文件仍为 `[]`；`--check` 与手工 regressions 条目不再冲突（本文档 §4 工作流不变，replay 测试已覆盖 regressions）。
- M2-F1/M2-F2 修复的工作树状态→提交事项已解决：修复已随 0.13.0 落地 commit 094f5d1/7e9de38 提交入库（§8 记录 session 8 结束时的 workspace 门禁 1,617 passed / 0 failed；CHANGELOG.md 的 0.13.0 节 1,617 passed 条目同步）。
