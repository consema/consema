...
- 缺陷分级：路线图 §18.4（第 1689-1691 行：P0 数据破坏/静默损失/RCE/错误写文件/跨快照误编辑；P1 panic/crash/hang/错误完成状态/明显语义不一致/limit bypass；P2 有安全替代路径的功能缺陷/非核心性能回退/诊断位置错误）
...
> 2026-08-13 11:19:44 session 916（waves.log 第 16343 行起）wave 1 启动 34 进程后驱动无记录终止——waves.log 止于 `wave 1 start`，无 wave done/session done 行、无 ABORT 行，runs.csv 零行入账（`session done: 122477 ledger rows` 已由前一会话 915 于 11:19:09 落账）。**已诊断并修复**（rs 波 2 审计）：根因为 PowerShell 脚本块 DataReceived 回调在 .NET 线程池上的未处理 PSInvalidOperation（`ScriptBlock.GetContextFromTLS` 竞争；WER 签名、exit code 2 无记录）——与 session 916 的终止同类；修复已落地于 consema-rs 的 `run_waves.ps1`（工作树未提交）：异步排空改为编译型 C# 委托（无 runspace 上下文，该崩溃类不再存在），并新增 ABORT 路径记录（会话未达 FINAL——强杀/未处理错误/提前退出——必须留下显式 ABORT 行，不再静默断档）。非 fuzz finding：不影响任何 target 的 release-candidate clean 计时、无 corpus regression（§4 step 2）。
...
新发现协议：P0/P1 = 立即报告 + 最小输入入 corpus + 清零该 target 时钟；P2 = 记录并给发布判断。`1.0.0` 不允许未解决 P0/P1（§18.4 第 1695 行）。
