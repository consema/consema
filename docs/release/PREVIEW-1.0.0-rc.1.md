...
| cargo-package-1.0.0-rc.1.log | `cargo package --workspace --locked --no-verify` 完整输出：15 个 crate 全部 Packaging 成功（14 个 publishable + repository-only 的 consema-conformance；consema 854.0KiB / consema-conformance 1.4MiB 等）；仅"manifest has no documentation, homepage or repository"warning | PowerShell 调 `cargo package --workspace --locked --no-verify 2>&1 \| Tee-Object` |
...
- cargo package：15 个 crate Packaging 成功（14 个 publishable + repository-only 的 consema-conformance，不发布）
