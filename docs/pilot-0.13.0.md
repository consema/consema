...
（exit 2）。无求值语义在 SDK 层（SECURITY.md，`hcl.expression@1` 只承载
...
（SECURITY.md，生成字节必先重解析）。同族 convert 一并执行成功：
...
| # | 指标 | 数值 | 测量方式 |
|---|---|---|---|
| 1 | exact unmodified round-trip rate | 2/2 = 100%（语义往返） | JSON→TOML→JSON 与 JSON→YAML→JSON 再转换值与源深度相等；字节级 round-trip 是库级门禁（18 套 suite 519/519，README.md；2026-08-12 P2-B 补强前为 508/508），CLI 可观察代理 = 目标字节重解析闭包（convert 每次成功都执行） |
...
#53 同一根因，见 `consema-rs/consema-document/src/source.rs`）。

修复（三处配合，各带回归测试）：

- `consema-rs/consema-yaml/src/offsets.rs`——`RawByteResolver`：单遍正向
  行走的 decoded-scalar → raw-byte 偏移解析器，每次 lookup 摊还 O(1)，
  O(source + lookups) 总量；与 `raw_byte_at` 逐点相等测试（:90-156）。
- `consema-rs/consema-document/src/source.rs`——`DecodedStorage::RawUtf8`
  保留构造时已校验的文本，`decoded_text`/`raw_byte_at` 变 O(1) 视图；
  回归网 `per_call_coordinate_conversion_does_not_rescan_large_utf8_sources`
  （:1538-1589）。
- `consema-rs/consema-xml/src/parser.rs`——xml 的 `raw_offset` UTF-8
  恒等快捷（同根因的 xml 侧修复）。
...
与 CHANGELOG.md 的口径（"materialize/convert 的目标字节默认只到 stdout；
