...
# python.exe）之前——钉版工件在本仓任何 checkout 都不存在，若先做文件检查，
# EAP=Stop 下缺文件错误使脚本以 exit 1 终止，skip 路径不可达（对照
# run-hcl-go-oracle.ps1 体例，skip 检查同样在工具链探测之前）。
$windowsBuild = [Environment]::OSVersion.Version.ToString()
if ($windowsBuild -cne $manifest.runtime.windows_build) {
    Write-Output "ConfigParser oracle: SKIPPED (Windows build mismatch: expected $($manifest.runtime.windows_build), got $windowsBuild; documented skip path, 2026-08-15 波 4 前置可及化 对照 run-hcl-go-oracle.ps1 体例)"
    exit 3
}
