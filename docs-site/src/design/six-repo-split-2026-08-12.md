...
- **向量变更是五仓同步事件**：任何一仓修改 `conformance/vectors/` 必须同步全部五个语言仓并更新聚合 digest 与 18/519 计数（README 声明；未同步变更在钉定移动前不会被语言仓 CI 自动捕获——go/ts/kt/py 四仓 CI 钉定 commit ccc9943（2026-08-15 波 4 R5 统一 provision 钉、F2 再锚母仓 HEAD；fc-manifest sha256 5cb4ab51）、consema-rs 为 vendored 快照，均不自动跟随）。
