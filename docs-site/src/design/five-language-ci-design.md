...
- **差分 harness 现状（关键事实）**：`scripts/go-verify-byte-parity.ps1`、`go-verify-normalized-
  ...
  CI**（.github 全域检索 `go-verify` 零命中）；按 consema-go/go/README.md:821-850 的既定分工，.github 是 Rust
  门禁域，Go 的验证以本地执行 + 文档化完成路径为准。差分证据链：byte parity 68/68（51 PVCE + 17
  PGCE，consema-go/go/README.md:776-798）、normalized 108/108 双向、protocol exchange 83/83 双向（
...
| semver（cargo-semver-checks，:201-235） | Rust API 稳定性 | **Rust 专属**。三语言 API 稳定性门禁属 1.0.0 收口（路线图 §22.2"Rust/Go public API 都完成稳定性审查"扩展至五语言；multi-language-implementation-plan.md:111）：TS exports 冻结审查 / Python `__all__` 审查 / Kotlin public API dump（kotlinx binary-compatibility-validator 属 dev 依赖，政策允许），全部 L5+ |
...
- 流水线照 go-verify-byte-parity.ps1:11-21 平移：cargo build（--locked -p consema-conformance
  ...
  re-encode，照 differential_test.go 体例，consema-go/go/README.md:781-785）。
- 环境变量：沿用 `CONSEMA_DIFFERENTIAL_RUST_DIR` 命名（它就是 Rust 侧 golden 目录，语义不变）；
  语言侧测试缺变量时 documented skip、绝不静默（consema-go/go/README.md:783-785）；脚本断言测试 RUN 而非
  SKIP（go-verify-byte-parity.ps1:117-124）。
- 每语言同时覆盖向量文件内 `pvce.*`/`pgce.*` hex 字段（portable-graph-v1.json 的固定字节，
  consema-go/go/README.md:567-589）——两条 golden 路径互补。
...
- **现状（已执行 2026-08-12）**：三份 case 文件原在 `go/conformance/differential/` 下（68/108/83，
  实测）。语言无关的 case 集（kind/format/profile/source/steps，consema-go/go/README.md:776-781）放在某语言
...
2. **多 agent 文件域纪律**：go-implementation-plan.md:235-237 与 consema-go/go/README.md:821-822 确立 ".github 是
...
| L5（runner 全 519 + fuzz/bench/security + CLI beta） | `L-conformance` 断言**零 documented skip**；新增 `L-package`（§1.1 package 行）；fuzz/bench/security 冒烟进 CI（短时长）；三平台矩阵按 Go 先例走文档化完成路径（consema-go/go/README.md:814-850，"completion path documented, not a CI job"）或显式 3-OS 矩阵 job——二选一在 L5 批次记录（**已处置 2026-08-12：文档化完成路径，§10 记录**） |
...
无收益；其存在已满足 Go 最低版本 CI 验证，consema-go/go/README.md:493-497）。
...
| # | 风险 | 缓解 |
|---|---|---|
| R-1 | **跨平台**：新语言仅 ubuntu（估计） | 三语言均跨平台（无 Windows 专属 API 面）；单 OS 照 go-1-26 先例（ci.yml:321-331）；3-OS 全矩阵属 L5/1.0.0（路线图 §22.4），按 Go 先例文档化完成路径（consema-go/go/README.md:814-850）。差分脚本均为 pwsh（ubuntu runner 可用，ci.yml:90 先例） |
...
- 差分现状：`scripts/go-verify-*.ps1` 四个脚本、`consema-go/go/README.md:766-850`
...
- **L5 批次落地（2026-08-12）**：
  ...
  - **3-OS 矩阵处置 = 文档化完成路径（§5.3 L5 行"二选一"决策）**：ts/py/kt
    ...
    完成路径 = 未来若出现平台专属需求，按 Go 先例（consema-go/go/README.md:814-850）
